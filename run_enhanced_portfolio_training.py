...
def save_results(results, output_dir, tickers, enhancement_config):
    ...
    try:
        ...
        # Importante: Aggiungi anche i valori finali dal test
        # Questo è un workaround - in una implementazione migliore, dovresti fare una 
        # verifica più completa dopo il test finale
        test_metrics_file = f'{output_dir}/test/test_metrics.json'
        if os.path.exists(test_metrics_file):
            try:
                ...
                # Aggiungi i valori di test come colonne separate
                results_dict['test_return'] = [test_metrics.get('total_return', 0)]
                results_dict['test_sharpe'] = [test_metrics.get('sharpe_ratio', 0)]
                results_dict['test_portfolio_value'] = [test_metrics.get('final_portfolio_value', 0)]
            except Exception as e:
                print(f"Errore nel caricamento dei test metrics: {e}")
        ...
        # Save the DataFrame
        results_file = f'{output_dir}/training_results.csv'
...
def plot_enhanced_performance(results, output_dir, tickers, enhancement_config, test_data=None):
    """Create visualizations of training performance with enhanced metrics."""

    # Nel corpo della funzione plot_enhanced_performance
    if test_data is None:
        test_data = {}  # Inizializza come dizionario vuoto se non è fornito

    plt.figure(figsize=(15, 12))
    
    # Correzione: Assicurati che stiamo graficando tutti i dati
    def safe_plot(ax, data, title, xlabel, ylabel):
        """Plot data safely, ensuring we're showing all data points."""
        if isinstance(data, list) or isinstance(data, np.ndarray):
            ax.plot(data)
        elif isinstance(data, deque):
            # Converti la deque completa in lista
            ax.plot(list(data))
        else:
    ...
    if 'cum_rewards' in results and len(results['cum_rewards']) > 0:
        safe_plot(ax1, results['cum_rewards'], 'Cumulative Reward', 'Episodes (x5)', 'Mean Reward')
    else:
        ax1.text(0.5, 0.5, "No cumulative reward data available", ha='center', va='center')
        ax1.set_title('Cumulative Reward')
    
    # Plot portfolio values
    ax2 = plt.subplot(3, 2, 2)
    if test_data.get('portfolio_value_log') is not None and len(test_data['portfolio_value_log']) > 0:
        # Usa i dati di test invece dei dati di training
        portfolio_values = test_data['portfolio_value_log']
        ax2.plot(portfolio_values, linewidth=2, color='darkblue')
        ax2.axhline(y=portfolio_values[0], color='r', linestyle='--', alpha=0.5)
        ax2.set_title('Test Portfolio Value', fontsize=14)
        ax2.set_xlabel('Step', fontsize=12)
        ax2.set_ylabel('Value ($)', fontsize=12)
        
        # Aggiungi informazioni sul rendimento
        if len(portfolio_values) > 1:
            return_pct = (portfolio_values[-1]/portfolio_values[0] - 1) * 100
            ax2.text(0.02, 0.95, f"Return: {return_pct:.2f}%", 
                    transform=ax2.transAxes, fontsize=10,
                    bbox=dict(facecolor='white', alpha=0.8))
    else:
        ax2.text(0.5, 0.5, "No test portfolio value data available", ha='center', va='center')
        ax2.set_title('Test Portfolio Value')
    
    # Plot Sharpe ratios
    ax3 = plt.subplot(3, 2, 3)
    if 'test_metrics' in test_data and 'sharpe_ratio' in test_data['test_metrics']:
        # Usa il valore di Sharpe dal test
        test_sharpe = test_data['test_metrics']['sharpe_ratio']
        
        # Crea un grafico a barre semplice per il valore di test
        ax3.bar(['Test Sharpe'], [test_sharpe], color='navy')
        ax3.set_title('Test Sharpe Ratio', fontsize=14)
        ax3.set_ylabel('Sharpe Ratio', fontsize=12)
        
        # Aggiungi l'etichetta del valore
        ax3.text(0, test_sharpe, f"{test_sharpe:.4f}", ha='center', va='bottom', fontsize=10)
        
        # Imposta i limiti dell'asse y per una migliore visualizzazione
        ax3.set_ylim([0, max(2.0, test_sharpe * 1.2)])  # Almeno fino a 2.0 o 20% sopra il valore
    else:
        # Alternativa: se hai i dati salvati in training data
        if 'test_metrics' in test_data:
            metrics = test_data['test_metrics']
            if 'sharpe_ratio' in metrics:
                ax3.bar(['Test'], [metrics['sharpe_ratio']], color='darkblue')
                ax3.set_title('Test Sharpe Ratio')
                ax3.set_ylabel('Sharpe Ratio')
            else:
                ax3.text(0.5, 0.5, "No Sharpe ratio data available", ha='center', va='center')
    ...
    if 'cvar_values' in results and len(results['cvar_values']) > 0:
        safe_plot(ax4, results['cvar_values'], 'Conditional Value at Risk (CVaR)', 'Episodes', 'CVaR')
    elif 'diversification_metrics' in results and len(results['diversification_metrics']) > 0:
        safe_plot(ax4, results['diversification_metrics'], 'Portfolio Diversification', 'Episodes', 'Diversification Score')
    else:
        ax4.text(0.5, 0.5, "No risk metrics available", ha='center', va='center')
    ...
    if enhancement_config.get('use_market_regimes', False) and 'regime_changes' in results:
        safe_plot(ax5, results['regime_changes'], 'Market Regime Changes', 'Episodes', 'Regime ID')
    elif 'exploration_rates' in results:
        safe_plot(ax5, results['exploration_rates'], 'Exploration Rate', 'Episodes', 'Exploration Rate')
    else:
        # Aggiungi un grafico della varianza delle posizioni se disponibile
        position_log_file = f'{output_dir}/test/position_log.npy'
        if os.path.exists(position_log_file):
            try:
                position_log = np.load(position_log_file)
                variance_positions = np.var(position_log, axis=0)
                
                # Usa i nomi dei ticker invece degli indici
                if tickers and len(tickers) == len(variance_positions):
                    ax5.bar(tickers, variance_positions)
                else:
                    ax5.bar(range(len(variance_positions)), variance_positions)
                    
                ax5.set_title('Position Variance')
                ax5.set_xlabel('Asset')  # Cambiato da 'Asset Index' a 'Asset'
                ax5.set_ylabel('Variance')
                
                # Aggiungi rotazione alle etichette se necessario
                plt.setp(ax5.get_xticklabels(), rotation=45, ha="right", rotation_mode="anchor")
            except Exception as e:
                ax5.text(0.5, 0.5, "No exploration or position data available", ha='center', va='center')
                ax5.set_title('Trading Activity')
        else:
            ax5.text(0.5, 0.5, "No exploration data available", ha='center', va='center')
            ax5.set_title('Exploration/Activity')
...
def create_enhanced_environment(args, valid_tickers, aligned_dfs_train, norm_params_paths, max_steps, output_dir, config=None, calendar=None, **kwargs):
    ...
    if config and 'env' in config:
        ...
        # Mappa tra i nomi degli argomenti e i nomi dei parametri nell'ambiente
        param_mapping = {
            'position_stability_factor': 'position_stability_bonus_factor',
            'diversification_factor': 'diversification_bonus_factor',
            'trading_activity_factor': 'trading_frequency_penalty_factor'
        }
    
        for key, value in env_params.items():
            # Se esiste un mapping per questa chiave, usalo
            mapped_key = param_mapping.get(key, key)
            if mapped_key in env_config:
                env_config[mapped_key] = value
            elif key in env_config:  # Fallback al nome originale se non c'è mapping
...
def save_test_metrics(metrics, output_dir):
    """Salva le metriche del test in un file JSON separato."""
    import json
    
    test_metrics_dir = f'{output_dir}/test'
    os.makedirs(test_metrics_dir, exist_ok=True)
    
    # Converti valori numpy in float nativi per la serializzazione JSON
    ...
    with open(f'{test_metrics_dir}/test_metrics.json', 'w') as f:
        json.dump(serializable_metrics, f, indent=4)
    
    print(f"Test metrics saved in: {test_metrics_dir}/test_metrics.json")

def save_test_logs(action_log, position_log, portfolio_value_log, rewards_log, test_env, output_dir):
    """Salva i log dettagliati del test per analisi future."""
    import json
    import numpy as np
    ...
    # Converti le liste di array numpy in array numpy
    action_log_array = np.array([a.tolist() if isinstance(a, np.ndarray) else a for a in action_log])
    position_log_array = np.array([p.tolist() if isinstance(p, np.ndarray) else p for p in position_log])
    
    # Salva i log come file numpy (più efficienti per grandi dataset)
    np.save(f'{test_metrics_dir}/action_log.npy', action_log_array)
    np.save(f'{test_metrics_dir}/position_log.npy', position_log_array)
    np.save(f'{test_metrics_dir}/portfolio_value_log.npy', np.array(portfolio_value_log))
    np.save(f'{test_metrics_dir}/rewards_log.npy', np.array(rewards_log))
    
    # Crea anche un CSV con i dati principali
    if len(portfolio_value_log) > 0:
        df = pd.DataFrame({
            'step': range(len(portfolio_value_log)),
            'portfolio_value': portfolio_value_log,
            'reward': rewards_log if len(rewards_log) == len(portfolio_value_log) else rewards_log + [0] * (len(portfolio_value_log) - len(rewards_log))
        })
        
        # Aggiungi le posizioni per ogni asset
        for i, ticker in enumerate(test_env.tickers):
            position_values = [pos[i] if i < len(pos) else 0 for pos in position_log]
            df[f'position_{ticker}'] = position_values[:len(df)]
        
        df.to_csv(f'{test_metrics_dir}/test_details.csv', index=False)
    
    print(f"Test logs saved in: {test_metrics_dir}/")

def check_metrics_consistency(train_results, test_metrics, output_dir):
    """Verifica la coerenza tra le metriche di training e test."""
    import json
    
    # Estrai le metriche di training rilevanti
    if isinstance(train_results['final_portfolio_values'], deque):
        train_portfolio_value = list(train_results['final_portfolio_values'])[-1]
    else:
        train_portfolio_value = train_results['final_portfolio_values'][-1] if len(train_results['final_portfolio_values']) > 0 else 0
    
    if isinstance(train_results['final_sharpe_ratios'], deque):
        train_sharpe = list(train_results['final_sharpe_ratios'])[-1]
    else:
        train_sharpe = train_results['final_sharpe_ratios'][-1] if len(train_results['final_sharpe_ratios']) > 0 else 0
    
    # Estrai le metriche di test
    test_portfolio_value = test_metrics['final_portfolio_value']
    test_sharpe = test_metrics['sharpe_ratio']
    
    # Calcola le discrepanze
    portfolio_value_diff = abs(test_portfolio_value - train_portfolio_value)
    portfolio_value_diff_pct = portfolio_value_diff / (abs(train_portfolio_value) + 1e-8) * 100
    sharpe_diff = abs(test_sharpe - train_sharpe)
    
    # Il flag is_consistent come valore intero invece che booleano (0 o 1)
    # Questo risolve il problema di serializzazione
    is_consistent = 1 if (portfolio_value_diff_pct < 50 and sharpe_diff < 1.0) else 0
    
    consistency_report = {
        'train_portfolio_value': float(train_portfolio_value),
        'test_portfolio_value': float(test_portfolio_value),
        'portfolio_value_diff': float(portfolio_value_diff),
        'portfolio_value_diff_pct': float(portfolio_value_diff_pct),
        'train_sharpe': float(train_sharpe),
        'test_sharpe': float(test_sharpe),
        'sharpe_diff': float(sharpe_diff),
        'is_consistent': is_consistent  # Usato 0/1 invece di True/False
    }
    
    with open(f'{output_dir}/metric_consistency_report.json', 'w') as f:
        json.dump(consistency_report, f, indent=4)
    
    # Report di consistenza
    print("\n===== CONSISTENCY CHECK =====")
    print(f"Training Final Portfolio Value: ${train_portfolio_value:.2f}")
    print(f"Test Final Portfolio Value: ${test_portfolio_value:.2f}")
    print(f"Difference: ${portfolio_value_diff:.2f} ({portfolio_value_diff_pct:.2f}%)")
    print(f"Training Final Sharpe: {train_sharpe:.4f}")
    print(f"Test Final Sharpe: {test_sharpe:.4f}")
    print(f"Difference: {sharpe_diff:.4f}")
    
    if portfolio_value_diff_pct > 50:
        print("WARNING: Large discrepancy in portfolio values between training and test!")
    if sharpe_diff > 1.0:
        print("WARNING: Large discrepancy in Sharpe ratios between training and test!")
    
    # Quando leggiamo il report possiamo convertire indietro is_consistent in booleano
    consistency_report['is_consistent'] = (is_consistent == 1)
    ...
    profiles = {
        'active': {
            ...
            'env': {
                ...
                'position_stability_bonus_factor': 0.01,  # Basso bonus per stabilità
...
def main(args):
    ...
    # Carica i dati di test necessari
    test_data = {}
    test_portfolio_value_file = f'{output_dir}/test/portfolio_value_log.npy'
    test_metrics_file = f'{output_dir}/test/test_metrics.json'

    if os.path.exists(test_portfolio_value_file):
        test_data['portfolio_value_log'] = np.load(test_portfolio_value_file)

    if os.path.exists(test_metrics_file):
        with open(test_metrics_file, 'r') as f:
            test_data['test_metrics'] = json.load(f)

    # Ora passa test_data alla funzione
    plot_enhanced_performance(results, output_dir, valid_tickers, enhancement_config, test_data)
    ...
    if model_files:
        ...
        if last_model:
            ...
            if args.use_backtest_framework:
                ...
                save_test_logs(action_log, position_log, portfolio_value_log, rewards_log, test_env, output_dir)
                ...
                consistency_report = check_metrics_consistency(results, metrics, output_dir)
                if not consistency_report['is_consistent']:
                    print("\nATTENZIONE: Discrepanza significativa tra metriche di training e test!")
                    print(f"Differenza nel valore del portafoglio: {consistency_report['portfolio_value_diff_pct']:.2f}%")
                    print(f"Differenza nello Sharpe ratio: {consistency_report['sharpe_diff']:.4f}")
